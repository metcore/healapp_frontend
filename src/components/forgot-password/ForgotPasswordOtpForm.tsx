...
export default function ForgotPasswordOtpForm() {
  ...
  const handleOnSubmit = (e) => {
    ...
    setLoadingSubmit(true)
    api.post('/auth/forgot-password/verify-otp', {
      user_id: user_id,
      otp: e?.values?.otp
    }).then((response) => {
      toast.success("Akun berhasil diaktivasi, silahkan login")
      router.push(`/forgot-password/create-password?user_id=${response?.data?.data.id}&otp=${e?.values?.otp}`, undefined, { scroll: false });
          ...
          <Form onSubmit={handleOnSubmit}>
            <Card>
              <div className="d-flex flex-column gap-8 p-64">
                <h6 className="text-semibold">Lupa Password</h6>
